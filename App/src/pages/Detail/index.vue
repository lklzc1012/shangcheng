        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(info, index) in spuSaleAttrList" :key="info.id">
                <dt class="title">{{ info.saleAttrName }}</dt>
                <dd changepirce="0" :class="{active: attr.isChecked == 1}" class="hyperlink"
                    v-for="(attr, index) in info.spuSaleAttrValueList" :key="attr.id"
                    @click="changeActive(attr, info.spuSaleAttrValueList)">{{ attr.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addShopCart" class="hyperlink">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changeActive(attr, attrList) {
      // 通过遍历将所有属性值的isChecked赋值0，即不激活active类名
    ...
    changeSkuNum(event) {
      // 通过与数字相乘，任何非数字数据类型与数字相乘为NaN进行判断
      let value = event.target.value * 1
      // 判断数量是否有非数字类型数据或数字小于1
